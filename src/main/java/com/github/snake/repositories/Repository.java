...
public class Repository {
	...
	/**
	 * This list keeps every @Entity's class that extends GameboardObject (is part from the gameboard).
	 * If new entity is added or removed should be fixed manually.
	 * Used too loop gameboard entities for the common methods (like when generating the gameboard we
	 * need all the entities from the same gameId or when checking if specific location is already taken)
	 */
	private static final List<Class<? extends GameboardObject>> ENTITY_CLASSES = Arrays.asList(SnakeHead.class,
			SnakeBody.class, Exit.class, Barrier.class, NormalFood.class, SpecialFood.class, Enemy.class);
	...
	/**
	 * Loops the gameboard entities to find if any of them is meet the requirements:
	 * @param gameId - the game ID which we're cheking the position
	...
	public GameboardObject findAnyByGameIdAndPosition(Long gameId, Position position) {
		...
		return ENTITY_CLASSES.stream().map(classType -> findByGameIdAndPosition(gameId, position, classType))
	...
	public List<GameboardObject> findAllByGameId(Long gameId) {
		...
		return ENTITY_CLASSES.stream().flatMap(classType -> findListByGameId(gameId, classType).stream())
				.collect(Collectors.toList());
	}

	/**
	 * @param <T> - represends the expected .class for the entity to be returned (entityClass and <T> must be the same)
	 * @param gameId - the game ID which we're looking for connected gameboard entities
	...
	public <T extends GameboardObject> T findByGameIdAndPosition(Long gameId, Position position,
		...
		query.setParameter(1, gameId).setParameter(2, position.getRow()).setParameter(3, position.getCol());
	 ...
	 * if the entity ID is null it is persisted (created new record with generated ID
	 * else if the entity ID is not null it merges it with the already existing record
	 */
	@Transactional
